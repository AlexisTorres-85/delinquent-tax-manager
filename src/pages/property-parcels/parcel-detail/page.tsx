...
function UpdateStatusPopover({ parcel }: { parcel: Parcel }) {
  ...
  const isNewCase = !parcel.activeCase && !caseStarted;
  ...
  // Sync status/stage from parcel whenever the popover is closed so
  // it always opens pre-populated with the latest active case values.
  useEffect(() => {
    if (!open && parcel.activeCase) {
      setStatus(parcel.activeCase.status);
      setStage(parcel.activeCase.stage);
    }
  }, [open, parcel.activeCase]);

  const startDelinquencyMutation = useMutation({
    mutationFn: () =>
      apiFetch<unknown>(`${API_BASE}/api/CaseProcess/start-delinquency-process`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          parcelId: parcel.id,
          taxYears: selectedYears.sort((a, b) => a - b).join(','),
          userObjectId: objectId,
          note,
        }),
      }),
    onSuccess: () => {
      toast.success('Delinquency process started successfully.');
      setCaseStarted(true);
      queryClient.invalidateQueries();
      setNote('');
      setSelectedYears([CURRENT_YEAR - 1]);
      setOpen(false);
    },
  });
  ...
  const updateCaseStatusMutation = useMutation({
    mutationFn: () => {
      ...
      const selectedStage = selectedStatus?.stages.find((st) => st.name === stage);
      return apiFetch<unknown>(`${API_BASE}/api/CaseProcess/update-case-status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          parcelId: Number(parcel.id),
          caseId: Number(parcel.activeCase?.caseId),
          userObjectId: objectId,
  ...
  function handleSave() {
    if (isNewCase) {
      startDelinquencyMutation.mutate();
      return;
    }
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent className="w-100 p-0 rounded-xl border-r-4 border-r-app-secondary/50 overflow-hidden shadow-xl shadow-black/15" align="end">
        <UpdateStatusForm
          ...
          workflowLoading={caseLoading || startDelinquencyMutation.isPending || updateCaseStatusMutation.isPending}
          statuses={statuses}
          originalStatus={parcel.activeCase?.status ?? ''}
          originalStage={parcel.activeCase?.stage ?? ''}
...
function ParcelDetailContent({ parcel, isError, error, parcelNumber, activeTab, onTabChange }: { parcel: Parcel | null | undefined; isError: boolean; error: unknown; parcelNumber: string; activeTab: Tab; onTabChange: (tab: Tab) => void }) {
  ...
  const hasCase = parcel.activeCase !== null;
  const visibleTabs = TABS.filter((tab) =>
    hasCase || (tab !== 'Tax Payments' && tab !== 'Payment Plan Schedule')
  );
